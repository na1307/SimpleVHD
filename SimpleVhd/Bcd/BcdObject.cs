...
using static SimpleVhd.Bcd.Internal;
...
public sealed record class BcdObject {
    ...
    public void SetIntegerElement(BcdElementType type, long integer) {
        ArgumentOutOfRangeException.ThrowIfNegative(integer);
        setElement(type, integer, "Integer");
    }
        ...
        foreach (PropertyData property in deviceData.Properties) {
            switch (property.Origin) {
                ...
                case nameof(BcdDeviceLocateStringData):
                    return new BcdDeviceLocateStringData(type, addOptions, (uint)deviceData["Type"], (string)deviceData["Path"]);

                case nameof(BcdDeviceLocateElementChildData):
                    return new BcdDeviceLocateElementChildData(type, addOptions, (uint)deviceData["Type"], (uint)deviceData["Element"], getDeviceData((ManagementBaseObject)deviceData["Parent"]));
